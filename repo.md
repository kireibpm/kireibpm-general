...
| kireibpm-ui-designer | IN_PROGRESS | 632 | 199 | 2026-03-06 | Frontend alias + migracja backend `model.contract` + testowy batch `webapp` (`assertions`/`utils` imports) do `org.kireibpm`; dodatkowo batch: migracja `backend/webapp` klasy `controller.preview.PreservingCookiePathProxyServlet` do `org.kireibpm` + aktualizacja importów w `WebMvcConfiguration` (commit `b6d31632`), walidacja `./mvnw -pl backend/contract,backend/webapp -DskipITs test`: BUILD SUCCESS (374 testy) |
