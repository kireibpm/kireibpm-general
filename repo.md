...
| kireibpm-ui-designer | IN_PROGRESS | 628 | 203 | 2026-03-08 | Frontend alias + migracja backend `model.contract` + testowy batch `webapp` (`assertions`/`utils` imports) do `org.kireibpm`; batch 1: migracja `controller.preview.PreservingCookiePathProxyServlet` + importy `WebMvcConfiguration` (commit `b6d31632`), batch 2: migracja `generator.Formatter` do `org.kireibpm` + aktualizacja static importów w `ParametrizedWidgetFactory`, `TableWidget` i `FormatterTest` (commit `29860a93`); walidacja obu batchy `./mvnw -pl backend/contract,backend/webapp -DskipITs test`: BUILD SUCCESS (374 testy) |
